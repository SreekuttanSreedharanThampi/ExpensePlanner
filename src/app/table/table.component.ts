...
export class TableComponent implements OnInit {
  ...
  ngOnInit(): void {  
    // Getting all the rows of this table when the table is initialized
    this.datasource.getTableRows(this.monthYear, this.monthNummber, this.table.tableName).subscribe((res)=>{
      this.table.rows =[];
      for (let row of res){
    ...
    this.addRowForm = new FormGroup({
      date: new FormControl('', [Validators.required, Validators.pattern('[0-9]*'), daysInMonthValidator(this.monthYear, this.monthNummber)]),
...
// Getters to access form elements and forms itself
